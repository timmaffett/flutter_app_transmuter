...
Future<void> copyFilesFromJson(String jsonFilePath) async {
  try {
    ...
    for (final sourceFileName in copyFiles.keys) {
      ...
      await copyFileWithMetadata(sourceFilePath, fullDestinationPath);
    }
...
Future<void> copyFileWithMetadata(String sourcePath, String destinationPath) async {
  try {
    ...
    await destinationFile.setLastModified(sourceStat.modified);
    await destinationFile.setLastAccessed(sourceStat.accessed);
...
Future<void> main() async {
  ...
  await Directory('destination_dir').create(recursive: true);
  await Directory('another_dir').create(recursive: true);
